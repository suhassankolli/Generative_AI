...
This version is optimized to run faster:
- It **removes the orchestrator LLM call** (no extra "agent decides tools" step).
- It runs **quality/readability/performance** checkers **in parallel**, so total time is close to the slowest checker.

Progress feedback:
- **CLI**: shows a Rich progress bar with stage updates.
- **Jupyter**: widget includes a progress bar and status text.
